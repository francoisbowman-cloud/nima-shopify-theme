...
Última actualización: 7 de agosto de 2026 — por: Code
...
| 73 | **Gap real encontrado en Omni: no existía ninguna operación para agregar margen alrededor de un sujeto** (`autotrim` solo hace lo opuesto, recortar bordes vacíos) — necesario para corregir el hallazgo `edge-contact` de la decisión #72. A pedido de Brey, se construyó la operación nueva **`extend-canvas`** en el repo fuente de `image-toolkit` (recorta al contenido real y lo recentra en un lienzo nuevo dimensionado por `target_occupancy`, con el margen nuevo rellenable como transparente/color/degradado/imagen), expuesta en los 3 puntos de entrada del toolkit (CLI, operación núcleo, tool MCP) sin duplicar lógica — reutiliza los helpers ya existentes de `replace_bg.py`. 4 tests nuevos, suite completa 403 passed (5 fallas preexistentes no relacionadas, mismas de siempre). **[PR #28](https://github.com/francoisbowman-cloud/image-toolkit/pull/28) mergeado a `main`** (con autorización explícita de Brey, mismo patrón que el PR #27) | Code (implementación), Brey (autorización de merge) |
| 74 | **Tratamiento de prueba aplicado a la foto de "16Pcs Timothy Hay Treats" (remove-bg + replace-bg con el color `--soft` de la marca, `#F3ECE1`)** — resultado visual limpio: fondo unificado a la marca en vez del estudio crudo del proveedor, y como efecto secundario el modelo de segmentación excluyó al hámster (estaba parcialmente fuera de cuadro y en sombra, la causa real del "animal cortado" del reporte de Brey), dejando conejo + cuyo + producto bien encuadrados. **Todavía no subido a Shopify** — quedó como archivo de resultado local de Omni (filesystem efímero). Con el PR #28 ya mergeado (decisión #73), el siguiente paso desbloqueado es aplicar `extend-canvas` sobre este resultado para agregar margen prolijo antes de subir la versión final — no ejecutado todavía, pendiente de que Brey lo pida | Code |
| 75 | **Catálogo visual completo descargado y organizado localmente, y piloto de 3 productos completado de punta a punta hasta Shopify.** (1) Las 203 imágenes originales de los 24 productos con datos de imagen en `products_export_1.csv` se descargaron a `nima-catalog-images/` (6 batches de 5, salvo el último de 4), con `manifest.json`/`master-index.csv`/`master-index.json` por producto — los 5 productos sin ninguna URL de imagen en el CSV (Premium Cat Litter Mat, Dog Clothes Puppy Shirts, Dog Birthday Hat, Dog Water Bottle, Dog Car Seat Cover) quedaron registrados en `removed_no_image_data`, **sin tocar Shopify/AutoDS/inventario** — es solo un recorte del set de trabajo local. (2) Paquete piloto `pilot-01` armado con 3 productos (Anti-Splash Water Bowl, Donut Bed, Grooming Gloves): imágenes originales + `manifest.json` + `product-brief.json` (título, descripción, variantes, colores, materiales solo cuando estaban en el texto de origen) + hoja de contacto, con notas críticas de fidelidad por producto (ej. Anti-Splash: forma cuadrada/turquesa-crema, no convertir en recipiente redondo). Enviado a Brey para trabajar con ChatGPT. (3) Brey devolvió el paquete aprobado por ChatGPT (`nima-pilot-01-approved-for-code.zip`, 9 imágenes: refined/lifestyle/in-use × 3 productos) — verificado localmente antes de tocar Shopify: integridad de las 9 imágenes (ninguna corrupta), fidelidad visual comparada 1:1 contra el original de cada producto (coincide en forma/color/mecanismo en los 3 casos). (4) **Subida a Shopify ejecutada con autorización explícita de Brey, solo agregar (sin borrar ningún original del proveedor)**, orden exacto pedido por producto, verificado por GraphQL tras cada `productReorderMedia` y validado en vivo en `nimapets.com` (colección, página de producto, viewport móvil 375×812) — Anti-Splash quedó con 12 imágenes (9 originales + 3 nuevas), Donut Bed con 11 (8+3), Grooming Gloves con 7 (4+3). Registro de rollback completo (media IDs originales + nuevos, procedimiento de reversión con y sin borrado) en `nima-catalog-images/pilots/pilot-01/_shopify_rollback_pre_upload.json` y `_shopify_upload_result.json`. **De paso, se reprobó el bug conocido de `shopify-staged-upload-signature-bug` (memoria del proyecto) y salió resuelto**: la `policy` que devuelve `stagedUploadsCreate` ahora sí incluye `x-goog-credential` en `conditions`, y una subida real a GCS devolvió HTTP 201 — memoria actualizada para reflejar esto. No se tocaron precios, variantes, inventario, AutoDS ni el theme | Code (pipeline completo), Brey (aprobación y coordinación con ChatGPT) |
| 76 | **Diagnóstico entregado (sin implementar nada todavía) para "Nima Catalog API Pipeline v0.1"**: reemplazar el paso manual de generación visual (ChatGPT vía Brey) por un pipeline local en Python que use la API de imágenes de OpenAI directamente, aislado en `tools/nima-catalog-ai/` dentro de este mismo repo (no hay stack Node real que reutilizar — el theme es Liquid vanilla sin build step, y el precedente de tooling auxiliar del repo, `image-server/`, ya es Python). Verificado contra documentación oficial de OpenAI del 05/08/2026 (no asumido de memoria, por instrucción explícita): modelo propuesto `gpt-image-2` (lanzado 21/04/2026), endpoints `images.generate`/`images.edit` (este último acepta múltiples imágenes de referencia — clave para usar los originales como referencia de fidelidad), tabla de precios por tamaño/calidad capturada con fecha y fuente. Producto propuesto para la primera prueba real: **crate water bowl de acero inoxidable rosa** (batch-06, no es ninguno de los 3 ya publicados en la decisión #75), presupuesto tope $1 USD. Diagnóstico completo (A-L: repo/carpeta, rama base, arquitectura a reutilizar, archivos a crear, SDK/endpoints, modelo verificado, estrategia de imágenes/JSON estructurado/costo, producto de prueba, riesgos, qué no se toca) entregado como archivo (`nima-catalog-ai-diagnostico.md`) — **sin crear rama, sin escribir código, esperando confirmación de Brey** sobre 4 puntos abiertos (Python vs. npm, producto de prueba, presupuesto, si el análisis de Fase 1 corre en `--dry-run`) | Code |
| 77 | **"Nima Catalog API Pipeline v0.1" implementado, testeado y validado con dos corridas reales contra la API de OpenAI — cerrado técnicamente, sin publicar en Shopify.** Autorizado por Brey (no el producto originalmente propuesto en la decisión #76 — se usó `waterproof-pet-feeding-mats-...`, batch-02, ya usado en el dry-run). Implementación completa en `tools/nima-catalog-ai/` (Python, sin stack Node, según lo diagnosticado): análisis de producto (Fase 1, `gpt-5.6-sol` con visión), `generation-plan.json` determinístico (sin llamada extra a la API), generación de imagen vía `gpt-image-2` (`images.edit`), fidelity gate automático, control de coste/intentos, y paquete de revisión local (contact sheet, nunca aprobación automática de Shopify). 49 tests (mocks, sin llamadas reales), commit `ccc4b6b` en `feat/nima-catalog-api-pipeline-v01` — **rama no mergeada a `main`, nada tocado en Shopify**. Dos corridas reales, $0.09 USD de gasto total: (1) primera corrida sin estrategia especial — `refined` y `lifestyle` ambos `reject` (pestaña con forma de pata mal reproducida, wordmark grabado "Moki Found" salió mal escrito "Hoki Found"); (2) segunda corrida con la estrategia **`product-preserving`** agregada a pedido de Brey — para `refined`, máscara de preservación a nivel de píxel (`src/masking.py`, heurística de color de fondo sin ML) que bloquea los píxeles del producto y solo permite editar el fondo, más un paso de recorte determinista para acercar la ocupación del producto al rango 75-88% (definido contra el CSS real de la tarjeta de catálogo, `theme/assets/base.css:221`) — **resultado: `refined` pasó a `review` (score 92, antes 58)**, wordmark y conteo de dedos correctos por primera vez; el recorte no pudo aplicarse porque la foto original no tiene margen lateral suficiente (limitación de la fuente, no del código, documentada). `lifestyle` (sin máscara — la escena completa cambia, una máscara de un solo cuadro no aplica) siguió en `reject` (mat mostrado ~25-30" en vez de 19", y el perro apareciendo en interacción activa en vez de presencia pasiva/ambiental) — **decisión explícita: no generar más intentos de `lifestyle` con el mismo método, documentado como límite conocido de v0.1**, no como bug a repetir. Se agregó además `product-overrides.json` (esquema propio, `schemas/product-overrides.schema.json`) para registrar correcciones humanas verificadas (texto exacto de wordmark, conteos de piezas, colores confirmados) sin editar nunca `product-analysis.json` a mano — se combinan solo al construir el plan de generación, con invalidación de caché en dos niveles (los overrides no fuerzan un nuevo análisis, pero sí invalidan el plan/estado de outputs cacheado). Limitaciones de v0.1 documentadas en el README del tool: el encuadre de `refined` depende del margen disponible en la foto fuente, no hay outpainting, `lifestyle` sin máscara puede alterar escala y confundirse con `in-use` | Code (implementación + validación), Brey (autorización de cada corrida real, decisiones sobre estrategia) |
| 78 | **"Nima Catalog AI v0.2 — Protected Lifestyle Composition" implementado y testeado (120/120 tests), sin publicar en Shopify.** Nueva arquitectura para resolver el límite conocido de v0.1 (decisión #77): en vez de que la IA regenere el producto completo dentro de la escena lifestyle, ahora **el producto real se segmenta de la foto fuente y se compone de forma determinística** sobre un fondo generado por IA que nunca contiene el producto. Pipeline: segmentación/cutout (`src/segmentation.py`, heurística de color de fondo reutilizada de `masking.py` — pluggable para rembg/SAM más adelante) → `placement-spec.json` (ocupación/bbox/escala, 100% matemático, sin llamada a API — `src/placement.py`) → `scene-spec.json` (modelo de interacción explícito: `lifestyle` exige `interaction_level=0` y los 3 flags de contacto en `false`, o directamente lanza error al construirse — `src/scene.py`) → `background-request.json` (la IA solo recibe instrucción de generar el entorno, con la zona del producto reservada vacía — `src/background.py`, backend intercambiable vía `src/background_provider.py`) → compositor determinístico (`src/compositor.py`, pega los píxeles reales del producto, escala uniforme sin distorsión) → sombra de contacto (`src/shadow.py`) → **Composition Gate** (`src/composition_gates.py`, checks de geometría/escena antes del Fidelity Gate existente, que se mantiene sin tocar) → paquete de revisión extendido (`src/composition_review.py`, campo `generation_strategy: "protected-product-composition"`) → salida visual obligatoria (`src/visual_debug.py`: `visual-debug/01-08` + `composition-contact-sheet.jpg` etiquetado, para poder juzgar el resultado sin abrir código). Implementado en rama nueva `feat/nima-catalog-ai-v02-lifestyle-composition` (base `1bab58b`, el cierre de v0.1) — **v0.1 queda intacto y congelado**, ningún módulo/test existente se tocó (49/49 siguen pasando). Demo offline completa con fixtures sintéticos (`src/demo_v02.py`, sin depender de `nima-catalog-images/` ni de una corrida real de v0.1). **Ninguna llamada real a la API de OpenAI en esta implementación** — `OpenAIBackgroundProvider` existe solo como interfaz, lanza `NotImplementedError` si se invoca, a propósito. Commit `91c1cd4` | Code (implementación completa, sin intervención de Brey punto por punto) |
| 79 | **"Real Pilot 01" — primera imagen lifestyle real con la arquitectura v0.2, autorizada explícitamente por Brey (1 llamada API máximo).** Producto: `waterproof-pet-feeding-mats-with-high-lips-multiple-size-and-colors-for-dogs-cats-others-g` (el mismo de la decisión #77), reusando el análisis de v0.1 sin nueva llamada multimodal. Segmentación real confirmada correcta por inspección visual: aisló la alfombrilla completa (wordmark "Moki Found", pestaña de pata con 4+1 pads, gotas de agua) y **excluyó automáticamente la grilla de swatches de color** de abajo — el caso exacto que `masking.py` documenta como su razón de ser. **1 llamada real a `images.generate`** (`gpt-image-2`, 1024×1024, ~$0.03, sin reintentos) generó un fondo editorial cálido de sala de estar, limpio, sin producto/mascota/texto — evaluado visualmente, no por un clasificador automático (limitación documentada, no hay CV en v0.2 para esto). Composición ejecutada con el compositor real: **Composition Gate: PASS** (ocupación 34% exacta al objetivo, sin clipping, aspect ratio preservado). **Hallazgo visual real, el cuello de botella actual de v0.2**: el producto se ve "pegado"/parado en vez de apoyado en el piso — la foto fuente es una toma cenital (desde arriba) y al insertarse en una escena con perspectiva de nivel de ojos, no hay transformación de perspectiva que lo asiente en el plano del suelo. Es una limitación de arquitectura conocida y documentada (`README.md`, "No 3D perspective"), no un bug puntual. **Bug real encontrado y corregido en el mismo pilot**: `visual_debug._thumb()` perdía el canal alfa con un `.convert("RGB")` sin componer sobre blanco antes, haciendo que los píxeles descartados del cutout (los swatches) reaparecieran silenciosamente en el panel CUTOUT del contact sheet — el PNG real del cutout siempre fue correcto, era solo el thumbnail del debug. Corregido componiendo sobre blanco primero. De paso se agregó `OpenAIClient.generate_image()` (`images.generate`, texto-a-imagen, distinto de `edit_image()` que v0.2 nunca llama). 120/120 tests siguen pasando tras el fix. Commit `71cc6f2`. **No se hizo una segunda llamada a la API** — instrucción explícita de Brey, el pilot es diagnóstico, no búsqueda de imagen perfecta. Rama pusheada a `origin` (sin PR, sin merge) | Code (ejecución completa, con autorización de Brey para la única llamada real) |
...
**Punto de entrada más reciente (07/08, decisiones #78/#79) — Nima Catalog AI v0.2 (Protected Lifestyle Composition), implementado, testeado, y probado con 1 pilot real.**
Rama `feat/nima-catalog-ai-v02-lifestyle-composition` (base `1bab58b`, el cierre de v0.1),
HEAD `71cc6f2`, pusheada a `origin` (sin PR, sin merge a `main`). 120/120 tests. Estado exacto
al cierre de esta sesión:
- **Arquitectura nueva funcionando de punta a punta**: producto real segmentado + compuesto
  determinísticamente sobre un fondo generado por IA (que nunca ve el producto) — reemplaza el
  enfoque de v0.1 de regenerar todo desde texto. Composition Gate + Fidelity Gate (sin tocar) +
  Human Review, en ese orden.
- **1 pilot real ejecutado** sobre el mismo producto de la decisión #77
  (`waterproof-pet-feeding-mats-...`): segmentación correcta (excluyó la grilla de swatches
  automáticamente), 1 llamada real a `images.generate` (~$0.03, sin reintentos), Composition
  Gate: PASS. **Cuello de botella real identificado**: el producto (foto cenital) se ve
  "pegado"/parado en vez de apoyado en el piso al insertarse en un fondo con perspectiva de
  nivel de ojos — v0.2 no tiene transformación de perspectiva todavía, es la limitación
  arquitectónica principal a resolver en una v0.3.
- Un bug real de visualización (no de composición) encontrado y corregido en el mismo pilot
  — ver decisión #79.
- **No consumir más API de OpenAI en esta rama** — mismo criterio que v0.1, instrucción
  implícita de "no perseguir la imagen perfecta con una segunda llamada" ya aplicada.
- Rama **no mergeada a `main`, nada tocado en Shopify** — pendiente de autorización explícita
  y separada para cualquiera de las dos cosas.
- Ver decisiones #78/#79 para el detalle completo, y `tools/nima-catalog-ai/README.md`
  (sección "Nima Catalog AI v0.2 — Protected Lifestyle Composition") para la arquitectura y
  limitaciones documentadas.

**Punto de entrada anterior (06/08, decisión #77) — pipeline v0.1, cerrado técnicamente, sin publicar en Shopify.**
`tools/nima-catalog-ai/` (rama `feat/nima-catalog-api-pipeline-v01`, commit `ccc4b6b`,
**congelada, no tocar**): `refined` llegó a `review` (score 92) pero no se publicó (encuadre
por debajo del rango objetivo, limitación de la foto fuente); `lifestyle` quedó en `reject`
(escala e interacción incorrectas) — este es el límite exacto que v0.2 fue construido para
resolver. Ver decisión #77 para el detalle completo.

**Prioridad previa, todavía sin resolver: hay TRES themes duplicados sin publicar en paralelo, todos partiendo
...
un índice rápido.)*
- **Nima Catalog AI v0.2 implementado y probado con 1 pilot real (decisiones #78/#79)** —
  pendiente de Brey: (a) decidir si vale la pena resolver el problema de perspectiva (producto
  "pegado"/parado en vez de apoyado en el piso) antes de seguir invirtiendo en este pipeline —
  es la limitación arquitectónica principal ahora mismo, no un detalle menor; (b) autorizar
  merge de `feat/nima-catalog-ai-v02-lifestyle-composition` a `main` (o seguir iterando en la
  rama); (c) autorizar explícitamente cualquier llamada API adicional o publicación a Shopify —
  ninguna imagen de v0.2 se subió, y solo se hizo 1 llamada real hasta ahora.
- **Pipeline "Nima Catalog API Pipeline v0.1" cerrado técnicamente, rama congelada (decisión #77)** — su límite conocido en `lifestyle` es la razón de ser de v0.2 (arriba). No retomar v0.1
  directamente — cualquier mejora de `lifestyle` debería pasar por v0.2 en adelante.
- **21 productos del catálogo (de los 24 con imágenes descargadas) siguen sin pasar por el
  pipeline visual** — solo los 3 de `pilot-01` (decisión #75) están con imágenes IA aprobadas
  en Shopify. El resto sigue con las fotos crudas de proveedor.
- **5 productos sin ninguna imagen en `products_export_1.csv`** (Premium Cat Litter Mat, Dog
  Clothes Puppy Shirts, Dog Birthday Hat, Dog Water Bottle, Dog Car Seat Cover — ver
  `removed_no_image_data` en decisión #75) necesitan una fuente de imagen distinta (Shopify
  Admin, proveedor, o revisión manual) antes de poder trabajarse visualmente.
