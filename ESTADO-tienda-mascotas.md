...
| 70 | **Filtro por categoría + orden implementado en `main-collection.liquid`** (chips generados desde `product.type` real de la colección vía `collection.products \| map: 'type' \| uniq`, filtrado client-side en `global.js` sobre la grilla ya renderizada — sin apps ni Search & Discovery) y **orden por precio** vía `sort_by` nativo de Shopify (server-side, funciona con paginación). Strings nuevos agregados a `es.default.json`/`en.json` bajo `collections.*` (nada hardcodeado). CSS nuevo (`.filterbar*`) sigue los tokens de la decisión #58/#60 (`--space-*`, `--radius-pill`, `--line`). `shopify theme check` local: 0 errores nuevos. **Pendiente**: subir a un duplicado sin publicar y que Brey lo revise/publique (mismo flujo de siempre, ver sección 7) | Code |
