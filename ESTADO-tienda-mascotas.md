...
Última actualización: 6 de agosto de 2026 — por: Code
...
**Punto de entrada más reciente (06/08, decisión #77) — pipeline de imágenes por API de OpenAI, cerrado técnicamente, sin publicar en Shopify.**
`tools/nima-catalog-ai/` implementado, testeado (49 tests) y validado con dos corridas reales
($0.09 USD de gasto total) — commit `ccc4b6b` en `feat/nima-catalog-api-pipeline-v01`. Estado
exacto al cierre de esta sesión:
- **`refined`**: aprobado como validación técnica del pipeline (llegó a `review`, score 92,
  con la estrategia `product-preserving` + máscara). **No aprobado para publicar en Shopify**
  — encuadre/ocupación (59.8%) por debajo del rango objetivo, limitación de la foto fuente
  (sin margen lateral suficiente), no del código. No generar otro intento con la misma fuente.
- **`lifestyle`**: rechazado (`reject`, escala del producto incorrecta + interacción activa en
  vez de presencia pasiva). No generar otro intento con el mismo método — documentado como
  límite conocido de v0.1 (prompt hardening por sí solo no controla escala/tipo de interacción
  sin una máscara o composición protegida, que no aplica cuando cambia toda la escena).
- **No consumir más API de OpenAI en esta rama** — instrucción explícita de Brey.
...
- Ver decisión #77 para el detalle completo (estrategia `product-preserving`, `masking.py`,
  `product-overrides.json`, limitaciones documentadas en `tools/nima-catalog-ai/README.md`).
...
un índice rápido.)*
- **Pipeline "Nima Catalog API Pipeline v0.1" cerrado técnicamente (decisión #77)** — pendiente
  de Brey: (a) autorizar merge de `feat/nima-catalog-api-pipeline-v01` a `main` (o seguir
  iterando en la rama), (b) decidir si/cuándo se retoma con presupuesto nuevo para mejorar
  `lifestyle` o probar con una foto fuente con más margen para `refined`, (c) autorizar
  explícitamente cualquier publicación a Shopify — ninguna imagen de este pipeline se subió.
